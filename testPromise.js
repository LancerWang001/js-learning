...
MyPromise.race = function (iterable) {
  return new MyPromise((resolve, reject) => {
    ...
    const nonPromiseArray = promiseArray.filter(v => (
      !(v instanceof MyPromise) || v.status !== PENDING
    ));
...
MyPromise.prototype.finally = function (onFinally) {
  const ignoreFinally = () => void 0;
  onFinally = typeof onFinally === 'function' ? onFinally : ignoreFinally;
  return this.then((v) => { nFinally(); return v; });
};

MyPromise.deferred = function () {
  const defferrdResult = {};
  const promise = new Promise((resolve, reject) => {
    defferrdResult.resolve = resolve;
    defferrdResult.reject = reject;
  });
  defferrdResult.promise = promise;
  return defferrdResult;
};
